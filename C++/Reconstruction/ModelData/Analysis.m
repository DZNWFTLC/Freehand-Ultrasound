...
% PC2 = Input_matrix(PC2,1000,false);
%
% hold on
...
for mi = 1:663
    if mi > 99
        filename = ['Kidney0',num2str(mi),'.bmp'];
    elseif mi>9
        filename = ['Kidney00',num2str(mi),'.bmp'];
    else
        filename = ['Kidney000',num2str(mi),'.bmp'];
    end
    
    %     A = rgb2gray(imread('Kidney0005.bmp'));
    % A2 = A(100:400,350:830);
    A = rgb2gray(imread(filename));
    thresh=A;
    thresh(thresh>50)=50;
    value = sum(sum(logical(A-thresh)));
    if (value > 10000)
    A2 = A(start:stop,left+1:end-right);
    A3 = wiener2(A2,[15 15]);
    A3 = medfilt2(A3,[15 15]);
    J = adapthisteq(A3);
    ...
    Contrast = uint8(150 * BW);
    Enhanced = A+ [zeros(start-1,1180);zeros(stop-start+1,left),Contrast,zeros(stop-start+1,right);zeros(696-stop,1180)];
    else
    Enhanced = zeros(size(A));    
    end
    figure(1)
    imshow(Enhanced);
    imwrite(Enhanced,['Enhanced\',filename]);
    figure(2)
    Ave1 = histogram(A);
    %Ave2 = histogram(Enhanced+A)
end
...
% uint8Image = [zeros(start-1,1180);uint8Image;zeros(696-stop,1180)];
% imshow(uint8Image+A);
