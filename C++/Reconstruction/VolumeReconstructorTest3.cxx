...
Mat  readCSVfile() {
	...
	Mat traNsformations(309,12,CV_64F);
	int tr_i = 0;
	int i = 0;
	while (i<3000) {
			...
			//cout << "type" << type << endl;
			//cout << "Timestamp" << Timestamp << endl;
			//cout << "angles" << JointAngles_ECM << endl;
			//cout << "angles" << JointAngles_PSM1 << endl;
			//cout << "angles" << JointAngles_PSM2 << endl;
			//cout << "angles" << JointAngles_PSM3 << endl;
			//cout << "base" << Pose_Base << endl;
			//cout << "ECM" << Pose_ECM << endl;
			//cout << "PSM" << Pose_PSM << endl;
			//cout << "RCM" << Pose_RCM << endl;
			//cout << "Mount" << Pose_Mount << endl;
			//cout << "Workplace" << Pose_Workplace << endl;
...
int main(int argc, char** argv)
{
	...
	//________________________________________________________ Initialize Aruco ______________________________________________//
		...
		std::string outputVolumeFileName = "C:/IGSIO-master/IGSIO-master/data/Volume_xaris_10_9_300.mha";
		...
		int used_frames = 300;   // choose how many frames to use 
		...
		Ptr<aruco::DetectorParameters> parameter = aruco::DetectorParameters::create();
		//parameter->errorCorrectionRate = 0.95f;
...
*/
			...
			if (detection_happened) {
				...
				char name[100];
				if (images_count < 10) {
					snprintf(name, sizeof name, "C:/IGSIO-master/IGSIO-master/build/Debug/Kidney_10_9/Kidney000%d.bmp", images_count);
				}
				else if (images_count<100){
					snprintf(name, sizeof name, "C:/IGSIO-master/IGSIO-master/build/Debug/Kidney_10_9/Kidney00%d.bmp", images_count);


				}
				else if (images_count<1000) {
					snprintf(name, sizeof name, "C:/IGSIO-master/IGSIO-master/build/Debug/Kidney_10_9/Kidney0%d.bmp", images_count);
				...
				if (detectTumor) {
					...
					// medfilt2
					for (int i = 1; i < 20; i = i + 2) /// prev video with 30
					{
					...
					if (true) {
						...
						// Kidney
						final_kidney.copyTo(black(cv::Rect(787 - 614, 265 - 72, 900, 380)));

						// Tumor
						if (interest_keypoints.size() > 0 && frames_counter<250) {

							final_im.copyTo(black(cv::Rect(1170 - 614, 280 - 72, 200, 150)));
						}
				...
				if (visual) {
					...
					for (int k = 0; k < 3; k++) {
						for (int j = 0; j < 4; j++) {
							...
							if (k ==0  && j == 3) {
								...
								matrix4_Probe_Ref->SetElement(k, j, 1000.*inputTranformations.at<double>(frames_counter, transf_counter));
								...
								matrix4_Probe_Ref->SetElement(k, j, 1000.*inputTranformations.at<double>(frames_counter, transf_counter));
									...
									double z_prev = 1000.*inputTranformations.at<double>(frames_counter, transf_counter);
									matrix4_Probe_Ref->SetElement(k, j, z_prev );

								
								/*else {
									double z_prev = inputTranformations.at<double>(frames_counter - 1, 11);
									double z_after = inputTranformations.at<double>(frames_counter, 11);


									matrix4_Probe_Ref->SetElement(k, j, z_prev - 1000.*abs((z_prev - z_after)));
								}*/
					...
					matrix4_Probe_Ref->SetElement(3, 3, 1.f);


				//	cout << "Translation x " << matrix4_Probe_Ref->GetElement(0,3) << endl;
				//	cout << "Translation y " << matrix4_Probe_Ref->GetElement(1, 3) << endl;
				//	cout << "Translation z " << matrix4_Probe_Ref->GetElement(2, 3) << endl;
