...
for num = 1:Num_data
    ...
    CameraImage= rgb2gray(imread(['CameraFrames\',filename]));
        ...
        offset_0.offset = zeros(8,1);
        offset = optimvar('offset',8);
        optexpr = fcn2optimexpr(@Corner_Optimisation3, offset,'OutputSize',[1,1]);
        convprob = optimproblem('Objective', optexpr);
        [sol,fval,exitflag,output] = solve(convprob,offset_0);
